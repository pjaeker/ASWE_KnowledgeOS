...
- Recommend bounded roles for the next slice only when a clear technical slice exists and the role adds value without duplicating the main writer.
...
- `Recommended bounded roles for next slice` only when a clear technical slice and clear bounded-role value are present
...
- recommending bounded roles by habit instead of evidence or technical need
