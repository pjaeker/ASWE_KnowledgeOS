...
| vscode.extensions_digest | string | ◑ | `NONE` | `sha256:...` | Digest über vollständige Extensionsliste (für Größenlimits) |
| vscode.extensions_total | number | ◑ | `0` | `147` | Anzahl installierter Extensions |
| vscode.extensions_truncated | bool | ◑ | `false` | `true` | Liste ist gekürzt, Digest ist vollständig |
...
## Automation (Option B, Windows)

### Script (empfohlen)

Lege dieses Script im Repo ab:

- Pfad: `scripts/optionb/Export-ToolingSnapshot.ps1`

Run im Repo-Root:

```powershell
powershell -ExecutionPolicy Bypass -File scripts/optionb/Export-ToolingSnapshot.ps1
```

Output:

- `meta/state/tooling/<Project>_ToolingSnapshot_YYYYMMDD_Vx.yml`
- Konsole druckt zusätzlich einen `sha256:` Digest + PR‑Report Snippet.

### VS Code Task (optional, 1 Shortcut)

Füge in `.vscode/tasks.json` einen Task hinzu:

```json
{
  "label": "Tooling: Export Snapshot (OptionB)",
  "type": "shell",
  "command": "powershell",
  "args": [
    "-ExecutionPolicy",
    "Bypass",
    "-File",
    "scripts/optionb/Export-ToolingSnapshot.ps1"
  ],
  "problemMatcher": [],
  "group": "build"
}
...
> Hinweis: Task kann in CI später als „informational“ laufen, aber der Snapshot ist primär **lokale Evidence** (weil Extensions/IDE‑State nicht in CI sichtbar sind).
