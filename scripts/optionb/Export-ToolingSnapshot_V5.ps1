...
.PARAMETER CodexConfigPath
  Optionaler expliziter Pfad zu einer Codex-Konfigurationsdatei.
  Hat Vorrang vor CodexConfigSource.

.NOTES
  Script-Version: V5
  Diese Datei ist die fuenfte, parallel gefuehrte Version von Export-ToolingSnapshot.ps1.

.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\Export-ToolingSnapshot_V5.ps1

.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\Export-ToolingSnapshot_V5.ps1 -RepoPath "D:\Git\ASWE_KnowledgeOS" -Project "AgenticSWE_KnowledgeOS"

.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\Export-ToolingSnapshot_V5.ps1 -OutDir "D:\Git\ASWE_KnowledgeOS\meta\state\tooling" -ExtensionSource auto

.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\Export-ToolingSnapshot_V5.ps1 -CodexConfigSource auto
#>
...
param(
  ...
  [Parameter(Mandatory=$false)][ValidateSet("auto","repo","user","none")][string]$CodexConfigSource = "auto",
  [Parameter(Mandatory=$false)][string]$CodexConfigPath = ""
)
...
function Add-CodexCandidate(
  ...
  $seen[$key] = $true
  $list.Add([pscustomobject]@{ source=$source; path=$full }) | Out-Null
}

function Resolve-CodexConfigFile([string]$repoRoot, [string]$sourceMode, [string]$explicitPath) {
  $seen = @{}
  ...
  switch ($sourceMode) {
    "auto" {
      Add-CodexCandidate $candidates $seen "repo" $repoPath
      Add-CodexCandidate $candidates $seen "user" $userPath
    }
    "repo" { Add-CodexCandidate $candidates $seen "repo" $repoPath }
    "user" { Add-CodexCandidate $candidates $seen "user" $userPath }
    "none" {}
  }

  foreach ($c in $candidates) {
    $tried.Add(($c.source + ":" + $c.path)) | Out-Null
    if (Test-Path $c.path) {
      return @{ found=$true; source=$c.source; path=$c.path; tried=@($tried) }
    }
  }
  return @{ found=$false; source="NONE"; path=""; tried=@($tried) }
}

function Read-CodexConfigSafeSubset([string]$path) {
  $sandbox = "NONE"
  $approval = "NONE"
  try {
    $txt = Get-Content $path -Raw -Encoding UTF8
    if ($txt -match 'sandbox_mode\s*=\s*"(.*?)"') { $sandbox = $Matches[1] }
    if ($txt -match 'approval_policy\s*=\s*"(.*?)"') { $approval = $Matches[1] }
    return @{ ok=$true; sandbox=$sandbox; approval=$approval; error="" }
  } catch {
    return @{ ok=$false; sandbox="NONE"; approval="NONE"; error=$_.Exception.Message }
  }
}
...
$codexConfigSourceUsed = "NONE"
$codexConfigPathUsed = "NONE"

if ($CodexConfigSource -eq "none" -and [string]::IsNullOrWhiteSpace($CodexConfigPath)) {
  $notes.Add("Codex config lookup disabled (CodexConfigSource=none).")
} else {
  $codexCfg = Resolve-CodexConfigFile $repoAbs $CodexConfigSource $CodexConfigPath
  if ($codexCfg.found) {
    $codexConfigSourceUsed = $codexCfg.source
    $codexConfigPathUsed = $codexCfg.path
    $parsed = Read-CodexConfigSafeSubset $codexCfg.path
    if ($parsed.ok) {
      $codexSandboxMode = $parsed.sandbox
      $codexApprovalPolicy = $parsed.approval
    } else {
      $notes.Add(("Failed to parse Codex config at {0} (safe subset)." -f $codexCfg.path))
    }
  } else {
    if ($codexCfg.tried.Count -gt 0) {
      $notes.Add(("Codex config not found (tried: {0})." -f ($codexCfg.tried -join "; ")))
    } else {
      $notes.Add("Codex config not found (no candidate path).")
    }
  }
}
...
Write-Host ("  codex_cfg: " + $codexConfigPathUsed)
...
$null = $sb.AppendLine("  config_source: " + (Yaml-Escape $codexConfigSourceUsed))
$null = $sb.AppendLine("  config_path_used: " + (Yaml-Escape $codexConfigPathUsed))
