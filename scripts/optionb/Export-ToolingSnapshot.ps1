...
.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\Export-ToolingSnapshot.ps1

.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\Export-ToolingSnapshot.ps1 -RepoPath "D:\Git\ASWE_KnowledgeOS" -Project "AgenticSWE_KnowledgeOS"
#>
...
function Try-Run([string]$cmd, [string[]]$args) {
  try {
    ...
    return @{ ok=$true; out=($out | Out-String).Trim(); path=$p.Source }
  } catch {
...
if ($IncludePipFreezeDigest -and $pythonVersion -ne "NONE") {
  try {
    ...
    if ($pythonPath -like "*py.exe") {
      $pipOut = & py -3 -m pip freeze 2>$null
    } else {
      $pipOut = & python -m pip freeze 2>$null
    }
    if ($pipOut) {
...
if ($gitVer.ok) {
  ...
  if ($o.ok -and $o.out) { $repoOrigin = $o.out.Trim() }
}
...
$dateIso = Get-DateIso
$outAbs = Join-Path $repoAbs $OutDir
Ensure-Dir $outAbs
...
# Write file
$yaml = $sb.ToString()
Set-Content -Path $outFile -Value $yaml -Encoding utf8NoBOM
